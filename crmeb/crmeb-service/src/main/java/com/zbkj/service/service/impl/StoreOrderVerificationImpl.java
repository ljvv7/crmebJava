...
public class StoreOrderVerificationImpl implements StoreOrderVerification {
    ...
    public StoreOrderVerificationConfirmResponse getVerificationOrderByCode(String vCode) {
        ...
        if (null == existOrder)
            throw new CrmebException(Constants.RESULT_VERIFICATION_ORDER_NOT_FUND.replace("${vCode}", vCode));
        if (existOrder.getStatus() > 0)
            throw new CrmebException(Constants.RESULT_VERIFICATION_ORDER_VED.replace("${vCode}", vCode));
